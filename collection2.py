...
maxes = []
minis = []
for i, b in enumerate(durations):
    if b == max_len:
        maxes.append(i)
    elif b == min_len:
        minis.append(i)
...
for id in minis:
    courses_min.append(courses_list[id][
                           "title"])  # Допишите код, который берёт по id нужный курс из courses_list и получает название курса из ключа "title"

for id in maxes:
    courses_max.append(
        courses_list[id]["title"])  # По аналогии допишите такой же код для курсов максимальной длительности


def find_shortest_course(courses, durations):
    shortest = {}
    for item in courses:
        shortest[item] = min(durations)
    return shortest


def find_longest_course(courses, durations):
    longest = {}
    for item in courses:
        longest[item] = max(durations)
    return longest

# print(courses_max)
# print(find_shortest_course(courses_min,durations))
# print(find_longest_course(courses_max,durations))
